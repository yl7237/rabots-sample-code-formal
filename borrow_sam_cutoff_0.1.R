...
p.f <- function(sample_b, sample_c, sample_t, delta) {
  ...
  post_t <- postmix(nf.prior, n = ss_trt, r = sum(sample_t))
  
  prior.hist <- mixbeta(c(1, n_b * orr_low, n_b * (1 - orr_low)))
  
  sam_w <- SAM_weight(if.prior = prior.hist, theta.h = orr_low, method.w = "LRT",
                      delta = delta, # Clinically significant difference
                      data = sample_t # Compared arm data
                      )
  SAM.prior <- SAM_prior(if.prior = prior.hist, # informative prior
                         nf.prior = nf.prior, # sam-prior
                         weight = sam_w, # mixture weight of the sam prior
                         delta = delta
                         )
  SAM.post <- postmix(SAM.prior, n = sum(ss_trt), r = sum(sample_t))
  p.SAM <- decision(SAM.post, post_c) 
  sam_w_cum <- ess(SAM.post, method = "moment") - length(sample_t)
  
  # Return selected outputs
  return(c(p.SAM, sam_w_cum, sam_w))
}
...
simFunc = function(
  ...
  sam_w_cum <- sam_w <- c()
  test.freq <- c()
  res_SAM_tmp <- res_SAM_high <- res_SAM_low <- c()
  
  # Start simulations  
  # ind: 0: no-go; 1: in-determinant; 2: go
  for (i in 1:nk) {
    print(i)
    set.seed(i+2024)
    ...
    if (min(test.cutoff1, test.cutoff2) >= cutoff1 & p.diff == 1) {
      ...
      res_SAM_low    <- c(res_SAM_low, p.res[1])
    ...
    # Store results
    sam_w_cum   <- c(sam_w_cum,   p.res[2])
    sam_w       <- c(sam_w,       p.res[3])
    res_SAM_tmp <- c(res_SAM_tmp, p.res[1])
    
  }
  
  #----- Summarize results ------------------------------------------------------
  nk <- length(res_SAM_tmp)  # Update number of valid simulations (in case of errors)
  
  out <- c(
    prop.table(table(res_SAM_tmp))[2],
    table(res_SAM_low)[2] / nk,
    table(res_SAM_high)[2] / nk,
    prop.table(table(res_SAM_tmp))[1]
  ) * 100
  out <- t(data.frame(out))
  
  # Dose selection summary: IA1, IA2, Total
  dosepick1   <- 100 * c(sel.lower1/nk, sel.higher1/nk, 1 - (sel.lower1 + sel.higher1) / nk)
  dosepick2   <- 100 * c(sel.lower2/nk, sel.higher2/nk, (1 - (sel.lower1 + sel.higher1) / nk) - (sel.lower2 + sel.higher2) / nk)
  dosepicktot <- 100 * c(sel.lower1/nk + sel.lower2/nk, sel.higher1/nk + sel.higher2/nk,
...
t2 = Sys.time()
t2 - t1
## Time difference of 53.25563 secs
...
poc_sam = data.frame(S2$out)
rownames(poc_sam) = rownames_trueORR
colnames(poc_sam) = colnames_poc

dosepick_sam = data.frame(S2$dosepick)
rownames(dosepick_sam) = rownames_trueORR
colnames(dosepick_sam) = colnames_do

# save(poc_sam, file = "poc_sam.RData")
# save(dosepick_sam, file = "dosepick_sam.RData")
