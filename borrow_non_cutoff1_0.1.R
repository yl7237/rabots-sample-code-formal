...
simFunc <- function(
  ...
  test.freq <- res_Non_tmp <- numeric(0)
  res_Non_high <- res_Non_low <- numeric(0)
  ...
  for (i in 1:nk) {
    ...
    if (min(test.cutoff1, test.cutoff2) >= cutoff1 & p.diff == 1) {
      ...
      res_Non_high   <- c(res_Non_high, p.res)
      ...
      res_Non_low    <- c(res_Non_low, p.res)
    ...
    # Store results
    res_Non_tmp  <- c(res_Non_tmp, p.res)
  }
  
  #----- Summarize results ------------------------------------------------------
  nk <- length(res_Non_tmp)  # Update number of valid simulations (in case of errors)
...
t2 = Sys.time()
t2 - t1
## Time difference of 1.815498 mins
...
poc_non = data.frame(S2$out)
rownames(poc_non) = rownames_trueORR
colnames(poc_non) = colnames_poc

dosepick_non = data.frame(S2$dosepick)
